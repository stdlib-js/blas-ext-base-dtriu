...
*/
function dtriu( order, M, N, k, A, LDA, B, LDB ) {
	var s;
	var l;

	l = resolveLayout( order );
	if ( l === null ) {
		throw new TypeError( format( '33pFx', order ) );
	}
	if ( isRowMajor( l ) ) {
		s = N;
	} else {
	...
	if ( LDA < max( 1, s ) ) {
		throw new RangeError( format( '33pJL', s, LDA ) );
	}
	if ( LDB < max( 1, s ) ) {
		throw new RangeError( format( '33pGM', s, LDB ) );
	}
	addon( resolveOrder( order ), M, N, k, A, LDA, B, LDB );
	return B;
}
